...
V3 优化 GiB 级、16 通道密集回放：提前准备解码器、按信号批量写盘、缓存两级峰值概览，并合并界面进度更新。全部解码样本仍保存在磁盘缓存；近距离缩放和游标测量读取原始值。详见 [V3 性能验证](docs/v3-performance.md)。

大文件导入改用完整元数据扫描，省去逐帧报文对象与时间换算开销，并显示文件级进度；未变化文件复用会话内预检结果。DBC 在后台解析，多个通道共享未变化文件的解析结果，大量信号分批显示。详见 [导入性能验证](docs/import-performance.md)。
...
16 通道每帧解码、实际显示 16 条轨道的压力测试：

```powershell
.venv\Scripts\python.exe -m scripts.benchmark_v3 --size-mib 1024 --label v3-run1 --ui
```

结果 JSON、生成数据和界面截图保存在 `.test-data/v3`。重复测量请更换 `--label`，BLF 会复用；需要约 2 GiB 可用磁盘。也可设置 `CANFLOW_TEST_16CH=1` 后单独运行 `pytest tests/test_large_blf.py -k dense_sixteen -s`。

## 使用

1. 切换到“文件与信号配置”页，从“项目”菜单新建项目，在“项目 DBC 映射”中手动新增通道并指定 DBC。映射独立于 BLF；项目保存 DBC 路径和当前信号选择，不复制 DBC 或 BLF。
2. 在配置页点击“添加 BLF 文件”并多选记录文件。首次预检仍完整扫描每个文件，状态栏显示当前文件及百分比，可点击“停止”取消；取消或失败时保留已有文件列表。BLF 中未配置 DBC 的通道仍显示原始报文。
...
5. 切换到“波形”页查看曲线。波形标题栏显示游标时间，左侧显示已选信号的游标值，可搜索并滚动查看 30–40 个信号；默认只在图上显示前 3 条曲线，勾选列表中的信号可添加到波形，点击信号可聚焦。每条可见曲线使用独立 Y 范围，共享时间轴。图下方“采样点粗细”统一调整真实数据帧对应采样点的粗细，采样点之间的连接线保持细线；设置会自动记住。Ctrl+滚轮同步缩放可见轨道的纵轴。
6. 查采样点间隔时，先在左侧选中目标信号，点击图下方“测量采样间隔”，再在波形上依次点击两次。A、B 标记会吸附到该信号原始采样点，显示时间差 Δt；将鼠标停在结果上可看两点的采集时间和值。点击“清除标记”重新测量。
7. “显示与回放”默认收起，可在其中调整曲线设置并打开回放详情。时间轴、进度和原始报文默认隐藏，展开状态会自动记住。
8. 处理完成后勾选其他信号会自动补画历史。程序会重新读取 BLF，无需重新导入。若信号在该 BLF 中没有可解码数据，界面会说明原因。
